...
const LoginForm: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const backendUser = data.user as { email: string; name?: string; role?: string };

      const userForContext = {
        email: backendUser.email,
        name: backendUser.name ?? backendUser.email.split("@")[0], // ✅ ADD
        role: (backendUser.role as "admin" | "student") ?? "student", // ✅ ADD
        token: accessToken,
        provider: "email" as const,
      };
      // Use existing AuthContext login(user, token, rememberMe)
      login(userForContext, accessToken, rememberMe);
